...
def _parse_content_category_elements_if(cell: element.Tag) -> list[tuple[str, str, list[tuple[str, str]]]]:
    ...
    _prune_condition(), in that order. A cell containing only '—' (no exceptions) yields nothing.
    ...
    for group in _gen_content_category_groups(cell):
        ...
        if subject is None:
            ...
            continue
        condition = _prune_condition(concat_text_nodes(get_nodes(_strip_condition_wrapper(group[1:]))))
...
def _prune_condition(nodes: list[tuple[str, str]]) -> list[tuple[str, str]]:
    """Apply the first matching rule to a condition node list.

    Each condition matches an exact lead/tail text shape observed in the source's exception prose;
    an unmatched shape passes through untouched.
    Checks are mutually exclusive by construction (distinct lead/tail literals and/or tuple counts),
    so match order doesn't affect the result.
    ...
    if links is not None:
        return [('attribute is not in state', ''), *links]
    ...
    if links is not None:
        return [('at least one child', ''), *links]

    # "it is _LINK_" | "it is a _LINK_" -> [link]
    for pattern in ('it is _LINK_', 'it is a _LINK_'):
        pruned = _match_condition_pattern(nodes, pattern)
        if pruned is not None:
            return pruned

    pruned = _prune_condition_attribute_is(nodes)
    if pruned is not None:
        return pruned
...
def _prune_condition_attribute_is(nodes: list[tuple[str, str]]) -> list[tuple[str, str]] | None:
